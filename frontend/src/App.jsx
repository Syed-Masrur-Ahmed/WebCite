...
function App() {
  ...
  const handleFetch = async () => {
    ...
    try {
      const response = await axios.get(
        `https://webcite-71wh.onrender.com/api/search?q=${encodeURIComponent(inputId)}`
      );
  ...
  onNodeClick={(node) => {
    if (node?.doi) {
      const url = node.doi.startsWith('http') ? node.doi : `https://doi.org/${node.doi}`;
            ...
            {hoveredNode && (
              ...
              >
                ...
                <Typography variant="body2" color="text.secondary" gutterBottom>
                  ...
                  {(hoveredNode.authors && hoveredNode.authors.join(', ')) || 'N/A'}
                </Typography>
                {hoveredNode.doi && (
